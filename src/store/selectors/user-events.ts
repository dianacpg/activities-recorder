...
// Define a selector that extracts the relevant pieces of state
const selectUserEvents = (state: AppState) => state.userEvents;

// Create a memoized selector using createSelector
export const selectGroupedEvents = createSelector([selectUserEvents], (userEvents) => {
  const events = userEvents.allIds.map((id) => userEvents.byIds[id]);
  const isLoading = userEvents.loading;
