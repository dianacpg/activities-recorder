...
describe("DayItems Component", () => {
  ...
  const mockComponent = (
    <DayItems day={10} month="August" events={mockEvents} onDelete={vi.fn()} onUpdate={vi.fn()} />
  );

  const firstThreeEvents = mockEvents.slice(0, 3);
  const remainingEvents = mockEvents.slice(3);

  it("renders date and month", () => {
  ...
  it("renders up to 3 events for each day when rendered", () => {
